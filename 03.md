...
你的计算机是什么操作系统的？自己先弄懂。如果是Linux某个发行版，就跟我同道了。并且我恭喜你，因为以后会安装更多的一些python库（模块），在这种操作系统下，操作非常简单，当然，如果是OSX，也一样，因为都是UNIX下的蛋。只是widows有点另类了。
...
以Ubuntu14.04为例，一般只要装好了这个操作系统，里面就已经把python安装好了。可能是python2.7.6版本，不过，在我来看，不需要升级，虽然目前最高版本是python2.7.9（在64位的上面，默认也安装了python3，供使用者选择）。
